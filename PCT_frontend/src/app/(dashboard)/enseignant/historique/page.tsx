  ...
  return (
    <AppShell role="enseignant">
      <PageHeader
        ...
        actions={
          <Button variant="outline"><Download className="h-4 w-4 mr-2" />Télécharger récapitulatif</Button>
        }
      />
            ...
            <TableBody>
              {data?.data.map((a) => (
                <TableRow key={a.id}>
                  ...
                  <TableCell className="text-sm font-medium">{a.attribution?.cours?.intitule_ecue}</TableCell>
                  <TableCell>
              ...
              {!data?.data.length && !isLoading && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-muted-foreground py-8">Aucune activité déclarée</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
